...
- `/PageMark` は decoded-false `pageMarks` として `Document` model と export/app-core JSON に保存する。header count/stride/last-index、row family、entry count、flags、`lineStart`/`lineEnd` rows を expose し、exporter が raw streams を読まずに fallback pages と original layout evidence を比較できるようにする。
