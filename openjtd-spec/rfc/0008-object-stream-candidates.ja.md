...
Raw `documentTextLineRuleProjection` candidates は、rule 単位と graph-component 単位の diagnostic-only render admission gate も持つようになった。各 rule は component membership、LineMark coverage、orthogonal/topology readiness、endpoint text-attachment candidates、明示的 blockers (`document-text-grid-origin-reference-backed`、endpoint ownership、style role、paint order) を記録する。Summary gate は current PDF-backed `shanai_lan` state を `ruleCount:16`、`componentCount:6`、`orthogonalComponentCandidateCount:3`、`bothEndpointAttachmentWithinLineHeightRuleCount:0`、`promotionReady:false` として aggregate する。これにより、source-derived page origin、endpoint ownership、style role、paint order が decode される前に、視覚的に魅力的な単一 line-rule overlay を promote しないようにする。
