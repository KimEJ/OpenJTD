...
**クラス `0x0010` のサブタイプ（decoded:false）。** `w4` フィールドにより `03新旧（整備令）.jtd` で少なくとも 4 種類のサブタイプが識別される：
...
**`b0`/`b1` はセル境界座標（スケール・単位は decoded:false）。** `03新旧（整備令）.jtd` の 703 件の分析結果：
...
`03新旧（整備令）.jtd` で 2 種類の形式が観測された（計 92 件）。
...
`03新旧（整備令）.jtd` で 4 件のみ観測。常に `0x000e`（表行区切り）の後、かつ `0x001c/0x0010` 単列段落の直前に現れる：
...
行政法令草案サンプル `02案文・理由（整備令）.jtd`（解析済み LineMark レコード 43 件、`base-unit=16`）でも同一のパターンが確認される：43 件の `unit-start` のうち 25 件が `0x001c` レコード位置に完全一致する。
...
`03新旧（整備令）.jtd`（表が多い新旧対照文書）では、すべての `0x000e` が直前と直後に `0x001c` レコードを持つ。`text-control-context` 診断ツールで、すべての `0x000e` が `prev-control=0x001c`（クラス `0x0030`）および `next-control=0x001c`（クラス `0x0030`）を持つことが確認された。
...
`0x000a` は `03新旧（整備令）.jtd` に 210 件出現し、現在の全サンプルに存在する（ファイルごとの件数は 2〜4671）。`0x000e` とは異なり、`0x0030` レコード間のセル間位置に限定されない。コンテキスト分析：
...
- クラス `0x0010` の可変長レコードは `w4/w5` に共通サブヘッダーシグネチャ `0x0026 0x0005` を共有しているようだ（`論文様式.jtd` len=20 と `01要綱/02案文/04参照` len=17 で確認）。`04参照条文（整備政令）.jtd` の len=17 レコード（計 142 件、`w4=0x0026 w5=0x0005`）詳細分析では、`w6`/`w7`/`w8`/`w9`/`w10` の組み合わせによる 9 種類のペイロードパターンが識別される。`w6=1`（102 件）のとき `w7=0x01ec=492`、`w8=w10=0x01cc=460` が固定で、ハンギングインデントグループとみられる（1/10mm 単位なら 49.2mm/46mm）。`w6=0`（40 件）のとき `w7=0/2/4`、`w8` はほぼ 0 で通常段落（インデントなし）と一致。`論文様式.jtd` len=20 ではインデント付き続き行に `w10=0x0141=321` が現れ、約 32mm のハンギングインデントと整合する。`03新旧（整備令）.jtd` の `w4=0x002e` バリアント（18 件、len=13）は完全に固定（`w5=w6=1`, `w7=0xffff`, `w8=0`）で均一な単列レイアウトを示す。単位スケールとフィールド役割は未解読。11 件の行政・学術サンプル全体での `w4=0x0026 len=17` レコード 246 件のクロスサンプル分析では、`(w8, w10)` の大きさによる構造的な分割が新たに判明した：`(w8=w10=0x01cc=460)` は `04参照条文` にのみ出現し（常に `w6=1` を伴う）、`(w8=w10=1)` または `(w8=w10=0/2)` は `01要綱` と `02案文` に専属。`02案文（整備令）` の 21 件では `w7` が 0/1/2/6/8 をとる。`02案文（整備令）` で `w7` と後続テキストを照合した結果、`w7` は視覚的インデント深さと単純対応しない：`w7=0` は左揃えの法令見出し・条・号見出しに出現；`w7=1` は条文本文継続；`w7=2` は号リスト・別表（先頭全角スペースあり/なし混在）；`w7=6` は前文本文；`w7=8` は附則見出し。このパターンは `w7` が視覚的インデント数ではなく段落スタイル ID をエンコードしているという仮説と整合する。`w7` 値と一太郎の段落スタイル名の対応は未証明。`(w8, w10)` は文書種別識別子か短テキストではスタイル ID・参照条文長文では物理座標をエンコードしている可能性がある。物理単位スケールは未証明。
