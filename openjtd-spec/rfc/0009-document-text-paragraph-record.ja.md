...
これは RFC 0003 §COM テキストエクスポート観測（`shanai_lan` の表コンテキストで `0x001c/0x0030` 行ヘッダーと `0x000e` 行区切りが観測された）を裏付ける。`0x000e` レコード自体の構造（ペイロード、長さ）はまだ解析・モデル化されていない。
...
- `0x001c/0x0030` セルヘッダー間の `0x000e` 行区切りは特定済みだが、`0x000e` レコード自体の構造（ペイロード、長さ）は未解読。
