...
- [ ] Decode the exact semantics of `/PageLayoutStyle` nested `31xx..39xx` slot groups, including which active slot pairs correspond to facing pages, header/footer areas, and page-number fields.
...
- [ ] Replace the vertical fallback display-unit heuristic with a proven `/PageMark`/`/LineMark` mapping from original layout rows to `Document` text lines. Rows 73 and 74 look sentinel-like (`lineStart=lineEnd` with zero geometry), so they must not be blindly treated as visible pages.
...
- [ ] Verify or replace suspicious reference PDFs before treating them as full-document regressions: local `a6.pdf` appears to contain only the TOC page while `a6.jtd` starts with the title page; local `46.pdf` appears to render SummaryInformation-like metadata rather than document body content.
